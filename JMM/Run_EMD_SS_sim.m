...
for jj = 1:n_vel
    ...
    for kk = 1:nPeriod
        ...
        EMD_data(index).HR_mean_ss      = mean(abs(EMD_data(index).HR_Motion(1:end,:)));
        EMD_data(index).HR_mean_ts      = mean(abs(EMD_data(index).HR_Motion), 2);
        EMD_data(index).HR_mean_ss_avg  = mean(abs(EMD_data(index).HR_mean_ss));
    end
end
disp('DONE...')
...
subplot(2,1,1)
temp_freq = repmat((vel_mean*3.75), nPeriod,1 )./repmat(spatPeriod', 1, n_vel);
speeds = repmat(vel_mean*3.75, nPeriod,1 );

...
amp = 15;
wave = 30;
freqList = logspace(-1,1.9,20); % [Hz]
showplot = false; 
MCS_List = 4*amp.*freqList;
...
FREQ = cell(length(Freq),1);
for kk = 1:length(Freq)
    [mag,phs,r2,freqData] = EMD_sim_v2(Freq(kk),amp,wave,Gain(kk),Phase(kk),showplot);
    Head_MAG(kk,1)   = mag{1};
...
plot(MCS_List,abs(MAG{1,1}),'k','LineWidth',2,'Marker','none','MarkerSize',15)
[mm,midx] = max(abs(MAG{1,1}));
plot([MCS_List(midx) , MCS_List(midx)] , [0 , mm] , '--k','LineWidth',1)
...
xlim([0 800])
grid on
plot(MCS,abs(Head_MAG),'b-','LineWidth',2,'Marker','.','MarkerSize',25)
[mm,midx] = max(abs(Head_MAG));
plot([MCS(midx) , MCS(midx)] , [0 , mm] , '--b','LineWidth',1)
